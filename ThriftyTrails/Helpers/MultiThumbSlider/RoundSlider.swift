...
public struct RoundSlider<V: BinaryFloatingPoint>: View {
    // MARK: - Properties
    ...
    let displayBounds: ClosedRange<V>?
        ...
        DragGesture(minimumDistance: 0)
            .onChanged { gesture in
                ...
                if showValueOnEditing {
                    displayTitle = String(format: "%0.2f", Double(displayValue ?? value))
                }
            }
            .onEnded { _ in
    ...
    public var body: some View {
        VStack(spacing: 0) {
            ...
            ZStack {
                ...
                Circle()
                    .inset(by: lineWidth * 0.5)
                    .trim(from: 0, to: arcLength * CGFloat(normalValue))
                    .stroke(style: style)
                    .rotation(rotation)
    ...
    public init(
        ...
        displayBounds: ClosedRange<V>? = nil,
        lineWidth: CGFloat = 7,
    ...
    private func updateValue(with translation: CGPoint) {
        ...
        let newValue = value + V(delta) * sensitivity
